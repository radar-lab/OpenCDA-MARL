# -*- coding: utf-8 -*-
"""
MARL-specific PID Controller with corrected longitudinal control.

This is a fixed version of opencda/core/actuation/pid_controller.py
that correctly uses longitudinal PID gains for speed control.

The original OpenCDA controller has a bug where lon_run_step() uses
_lat_ebuffer and _lat_k_* (lateral gains) instead of _lon_* (longitudinal gains).
This causes poor speed tracking in MARL scenarios.
"""

# Copyright (c) 2018-2020 CVC.
# Modified for MARL by AXIBA (leolihao@arizona.edu)
#
...
class Controller:
    """
    PID Controller implementation with FIXED longitudinal control.
    ...
    def lon_run_step(self, target_speed):
        """
        FIXED: Now correctly uses _lon_ebuffer and _lon_k_* for longitudinal control.
        ...
        # FIXED: Use _lon_ebuffer instead of _lat_ebuffer
        ...
        # FIXED: Use _lon_k_* instead of _lat_k_*
