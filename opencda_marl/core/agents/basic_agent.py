...
from opencda_marl.core.plan import LocalPlanner, RoadOption
from opencda.core.plan.global_route_planner import GlobalRoutePlanner
from opencda.core.plan.global_route_planner_dao import GlobalRoutePlannerDAO
from opencda_marl.core.plan.misc import (get_speed, is_within_distance,
...
class BasicAgent(object):
    ...
    def __init__(self, vehicle, target_speed=20, opt_dict={}, map_inst=None, grp_inst=None):
        ...
        local_planner_opt_dict = opt_dict.get('local_planner', {})
        self._local_planner = LocalPlanner(
            self._vehicle, opt_dict=local_planner_opt_dict, map_inst=self._map)
        if grp_inst and isinstance(grp_inst, GlobalRoutePlanner):
            ...
            self._global_planner = GlobalRoutePlanner(
                dao, self._world, debug=self._debug)
    ...
    def set_target_speed(self, speed):
        ...
        self._local_planner.set_speed(float(speed))
    ...
    def set_destination(self, end_location, start_location=None, clean_queue=None):
        """
        ...
        which corresponds (by default), to a location about 5 meters in front of the vehicle.
            ...
            :param clean_queue (bool): whether to clean the waypoint queue. If None, auto-determined.
        """
        if clean_queue is None:
            # Auto-determine clean_queue based on start_location presence (backward compatibility)
            if not start_location:
                start_location = self._local_planner.target_waypoint.transform.location
                clean_queue = True
            else:
                # Use the provided start_location as-is, don't override with vehicle location
                clean_queue = False
        else:
            # Use explicit clean_queue parameter
            if not start_location:
                start_location = self._local_planner.target_waypoint.transform.location
        ...
        if self._debug:
            if not route_trace:
                print("BasicAgent: No route trace found")
                return
            # debug the route trace
            for waypoint, _ in route_trace:
                self._world.debug.draw_point(waypoint.transform.location, size=0.1, color=carla.Color(
                    r=0, g=255, b=0), life_time=1)
        # </OpenCDA-MARL> debug the route trace
