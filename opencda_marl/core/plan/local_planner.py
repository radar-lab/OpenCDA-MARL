...
Date         : 2025-09-01 11:47:32
FilePath     : /OpenCDA-MARL/opencda_marl/core/plan/local_planner.py
Description  :  MARL Specialized Local Planner
...
from .debug_helper import draw_trajetory_points

# --------------------------------------------------------------------- #
# Original Carla's Local Planner
# --------------------------------------------------------------------- #
...
class LocalPlanner(object):
    ...
    def __init__(self, vehicle, opt_dict={}, map_inst=None):
        ...
        self._args_longitudinal_dict = {
            'K_P': 1.5, 'K_I': 0.05, 'K_D': 0.2, 'dt': self._dt}
        ...
        self._debug = False # whether to draw the future/history waypoints
        self._debug_trajectory = False # whether to draw the trajectory points and path
        self._trajectory_update_freq = 15 # trajectory points updating frequency
        # Overload parameters
        if opt_dict:
            if 'dt' in opt_dict:
            ...
            if 'debug_trajectory' in opt_dict:
                self._debug_trajectory = opt_dict['debug_trajectory']
            if 'trajectory_update_freq' in opt_dict:
                self._trajectory_update_freq = opt_dict['trajectory_update_freq']
    ...
    def set_speed(self, speed):
        ...
        if self._follow_speed_limits:
            logger.warning("The max speed is currently set to follow the speed limits. "
    ...
    def run_step(self):
        ...
        if self._debug_trajectory:
            # Draw debug visualization like OpenCDA
            world = self._vehicle.get_world()

            # Draw current target waypoint (single waypoint)
            draw_waypoints(world, [self.target_waypoint], 1.0)

            # Draw waypoint buffer (blue points) - upcoming waypoints
            if len(self._waypoints_queue) > 0:
                # Get next self._trajectory_update_freq waypoints from queue for visualization
                waypoint_list = []
                for i, (waypoint, _) in enumerate(self._waypoints_queue):
                    if i >= self._trajectory_update_freq:  # Limit visualization
                        break
    ...
    def update_information(self, ego_pos, ego_speed):
        """
        Update ego position and speed for the local planner.
        This is essential for waypoint following and distance calculations.

        :param ego_pos: Current ego vehicle position (carla.Transform)
        :param ego_speed: Current ego vehicle speed (km/h)
        """
        # Update vehicle controller if it exists
        if self._vehicle_controller:
            # The PID controller needs current position and speed for proper control
            pass  # PID controller gets this from the vehicle directly

        # Update target waypoint if we have a queue and the vehicle has moved
        if len(self._waypoints_queue) > 0 and ego_pos:
            # This is handled in run_step() purging logic, but we ensure consistency here
            pass

    # --------------------------------------------------------------------- #
    # MARL Specialized Observation
    # --------------------------------------------------------------------- #

    # ADDON: get the direction of the vehicle
    def get_current_direction(self, steps=1):
        """Returns the current direction of the vehicle"""
        cwpt = self._map.get_waypoint(self._vehicle.get_location())
        wpt, _ = self.get_incoming_waypoint_and_direction(steps)
        if wpt is None:
            return RoadOption.VOID
        dir = _compute_connection(cwpt, wpt)
        return dir

    # ADDON: get the current lane of the vehicle
    # TODO: May need optimize the code
    def get_current_lane(self):
        """Returns the current lane of the vehicle"""

        cwpt = self._map.get_waypoint(self._vehicle.get_location())
        change = cwpt.lane_change

        # from loguru import logger
        # logger.debug(
        #    f"change ({change}), L({cwpt.get_left_lane()}), R({cwpt.get_right_lane()})")
        if change == carla.LaneChange.NONE:
            return 1
        # Means on the right lane
        elif change == carla.LaneChange.Left:
            return 2
        elif change == carla.LaneChange.Right:
            return 0
        # on turning lane
        else:
            return 1

    # ADDON: get the waypoint buffer size
    def get_waypoint_buffer(self):
        """Returns the size of the waypoint buffer"""
        return self._waypoints_queue
